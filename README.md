...
Информацияонная база уже включает в себя исходный код скрипта python, а также готовые шаблоны для формирования конфигурационного файла `config.json`. 
По шаблону анализа автоматически сформируется `config.json` и выполнится запуск скрипта python. Результаты анализа сразу будут доступы в каталоге с результатами.
...
Подробное руководство: [Запуск через информационную базу "Анализ технологического журнала"](#запуск-через-информационную-базу-анализ-технологического-журнала)
